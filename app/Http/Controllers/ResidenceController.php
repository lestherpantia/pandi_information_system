...
//            $birthday = new Carbon((int)$request->birthday_year . '-' . (int)$request->birthday_day . '-' . $month['month']);
    ...
    public function  pdf() {
        ...
        foreach($barangays as $barangay)
        {
            ...
            $senior = $voterList->where('senior', true)->count();
            $disability = $voterList->where('disability', true)->count();
            $solo_parent = $voterList->where('solo_parent', true)->count();
            $pregnant = $voterList->where('pregnant', true)->count();
            $lactating_mother = $voterList->where('lactating_mother', true)->count();
            $homeless = $voterList->where('homeless', true)->count();
            $none = $voterList->where('senior', false)->where('disability', false)->where('solo_parent', false)->where('pregnant', false)->where('lactating_mother', false)->where('homeless', false)->count();
